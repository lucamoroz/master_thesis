...
\begin{document}
	...
	% TODO prophet weekly/daily terms, deepar/state/mqcnn time  and static features
...
\end{lstlisting}
	...
	% TODO write about clustering and tuning: naive cgp on allianz performs better with meanshift, which creates more clusters. On the other size, non-naive cgp performs better with kmeans. Maybe because naives has more points available? (the number of errors differ by one)
	
	\clearpage
	\subsection{Forecasting} \label{ssec:results_forecasting}
	% todo dire che serie temporale allianz ha picchi rapidi
...
\end{table*}
	...
	Such property of the workload changes with a daily seasonality on the Daily02B scenario and a weekly seasonality on the Bank4 scenario, using a simple step function. 
	...
	\begin{table}\centering 
		...
		\begin{tabularx}{\textwidth}{@{}llllXX@{}}
			...
			\midrule
			Daily02&Daily.02 & $<9ms$  & 1 week & [req/s]  & $k$-means\\
			Daily04&Daily.04 & $<9ms$  & 1 week & [req/s]  & $k$-means\\
			Daily08&Daily.08 & $<9ms$  & 1 week & [req/s] & $k$-means\\
			Daily02B&Daily.02 & $<8ms$  & 1 week & [req/s, benchmark] & $k$-means\\
			Weekly&Daily.02 & $<10ms$  & 2 weeks & [req/s]  & mean shift\\
			Bank1&Bank & $<9ms$  & 1 week & [req/s]  & $k$-means\\
			Bank2&Bank & $<9ms$  & 1 week & [req/s]  & mean shift\\
			Bank3&Bank & $<10ms$  & 1 week & [req/s]  & $k$-means\\
			Bank4&Bank & $<10ms$  & 1 week & [req/s, benchmark]  & $k$-means\\
			Taxi1 &Taxi & $<9ms$  & 24 days & [req/s]  & $k$-means\\
			Taxi2 &Taxi & $<10ms$  & 24 days & [req/s]  & $k$-means\\
	...
	\begin{figure}\centering
		...
		\caption{Results of scenario Bank3.}
	...
	\begin{figure} \centering
		...
		\caption{Bank3 scenario Time To Recover. The yellow line is the Baseline CR. DeepAR, Prophet, and Naive online tuners overlap, leading to a TTR equal to 21 iterations, i.e. 1.5 hours.}
	...
	\begin{table*}\centering 
		...
		\begin{tabularx}{\textwidth}{@{}XXXXXX@{}}
			\toprule
			Time series & Tuner & $\Theta$/$\tilde{\Theta}$ & Precision & Recall& F1\\
			...
			\bottomrule
		\end{tabularx}
		\caption{Tuning scenarios stability data, valid both for Cassandra and MongoDB scenarios.} \label{table:results_stability}
	\end{table*}
	...
	The developed online tuners have proved the ability of finding better configurations than the baseline, either reducing the memory used by a software system or its response time. However, the exploration of the configuration space to find such better settings inevitably leads to failures on the production system both in terms of QoS or preventing it from serving its clients. Nevertheless, by using the developed forecasting and stability modules the overall number of failures can be reduced. 
	
	To further reduce the number of failures, multiple optimizations can be implemented. First of all, at the beginning of the tuning process the tuners suggest a configuration without any available knowledge, making the suggestion random. To overcome this issue, the tuner may sample the outcome of the baseline configuration under different workload types, so that the first suggestion uses a minimum amount of information. Note that a smart way to summarize the collected knowledge should be implemented. Then, we may tune the exploration-exploitation trade-off provided by the BO acquisition function explained in section \ref{ssec:bayesian_optimization} to boost exploitation, so that the tuners prefer to suggest configurations within a region that has already shown to be promising. Finally, to reduce the impact of failures on the production system, the deployment of the configurations could be performed using a canary deployment.
	
	To further improve the cumulative reward (e.g. overall response time of the system) during the tuning process, we may apply the best configuration found for the upcoming predicted workload rather than the baseline when the workload is predicted to be unstable. Such optimization should be applied once enough experiments have been collected. Then, when the tuners are no longer able to improve the best configuration found so far (or the improvements are minimal) we may stop the tuning process, starting to just apply the best configuration according to the workload.
	
	Besides these optimizations, we tested the tuners using univariate time series or simple multivariate time series due to a lack of data. In order to test the tuners on more complex scenarios, such as a system with multiple front-end and back-end nodes each with its own time series, more data must be collected, along with the creation of system models. 
	
	Finally, finding a good pair of thresholds to be used to evaluate the stability of the actual and predicted workloads is not trivial, and at the moment requires manual work. By developing an automated approach that chooses these thresholds we would simplify the process of applying the online tuners, reducing the risk of making mistakes.
